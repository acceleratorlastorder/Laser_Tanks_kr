...
data:extend({
    {
        type = "equipment-grid",
        name = CAR_EQUIPEMENT_GRID_NAME,
        width = 8,
        height = 8,
        equipment_categories = {"armor"}
    }, {
        type = "equipment-grid",
        name = TANK_EQUIPEMENT_GRID_NAME,
        width = 10,
        height = 9,
        equipment_categories = {"armor"}
    }
...
car.equipment_grid = CAR_EQUIPEMENT_GRID_NAME
if settings.startup["lasertanks-electric-engine"].value then
    table.insert(data.raw["equipment-grid"][CAR_EQUIPEMENT_GRID_NAME]
                     .equipment_categories, "electric-vehicles-equipment")
    car.burner = {effectivity = 1, fuel_inventory_size = 0}
...
tank.equipment_grid = TANK_EQUIPEMENT_GRID_NAME
if settings.startup["lasertanks-electric-engine"].value then
    table.insert(data.raw["equipment-grid"][TANK_EQUIPEMENT_GRID_NAME]
                     .equipment_categories, "electric-vehicles-equipment")
    tank.burner = {effectivity = 1, fuel_inventory_size = 0}
end
if data.raw.car["vehicle-laser-tank"] and
    settings.startup["lasertanks-aai-tank"].value then
    tank.animation =
        table.deepcopy(data.raw.car["vehicle-laser-tank"].animation)
    tank.turret_animation = table.deepcopy(
                                data.raw.car["vehicle-laser-tank"]
                                    .turret_animation)
end
-- rifle.attack_parameters.damage_modifier = 0.05--2 --submachinegun = 12.5
...
vehicle_laser_gun.attack_parameters.sound = {
    {filename = "__Laser_Tanks_kr__/test2.ogg", volume = 0.3}
...
tank_laser_cannon.attack_parameters.sound = {
    {filename = "__Laser_Tanks_kr__/cannon.ogg", volume = 0.5}

}
data:extend({
    car, car_recipe, car_item, tank, tank_recipe, tank_item, vehicle_laser_gun,
    tank_laser_cannon
})
...
if settings.startup["lasertanks-override-color"].value then
    ...
    data.raw.item["laserrifle-charger"].icons = {
        {
            icon = "__Laser_Tanks_kr__/graphics/charger2.png",
            tint = {r = r, g = g, b = b}
        }, {
            icon = "__Laser_Tanks_kr__/graphics/charger3.png",
